...
void Board::print_settings(ostream &os) const
{
  ...
  os << "NUM_DAYS"
     << "\t" << num_days() << endl;
  os << "NUM_ROUNDS_PER_DAY"
     << "\t" << num_rounds_per_day() << endl;
  os << "BOARD_ROWS"
     << "\t" << board_rows() << endl;
  os << "BOARD_COLS"
     << "\t" << board_cols() << endl;
  os << "NUM_INI_BUILDERS"
     << "\t" << num_ini_builders() << endl;
  os << "NUM_INI_WARRIORS"
     << "\t" << num_ini_warriors() << endl;
  os << "NUM_INI_MONEY"
     << "\t" << num_ini_money() << endl;
  os << "NUM_INI_FOOD"
     << "\t" << num_ini_food() << endl;
  os << "NUM_INI_GUNS"
     << "\t" << num_ini_guns() << endl;
  os << "NUM_INI_BAZOOKAS"
     << "\t" << num_ini_bazookas() << endl;
  os << "BUILDER_INI_LIFE"
     << "\t" << builder_ini_life() << endl;
  os << "WARRIOR_INI_LIFE"
     << "\t" << warrior_ini_life() << endl;
  os << "MONEY_POINTS"
     << "\t" << money_points() << endl;
  os << "KILL_BUILDER_POINTS"
     << "\t" << kill_builder_points() << endl;
  os << "KILL_WARRIOR_POINTS"
     << "\t" << kill_warrior_points() << endl;
  os << "FOOD_INCR_LIFE"
     << "\t" << food_incr_life() << endl;
  os << "LIFE_LOST_IN_ATTACK"
     << "\t" << life_lost_in_attack() << endl;
  os << "BUILDER_STRENGTH_ATTACK"
     << "\t" << builder_strength_attack() << endl;
  os << "HAMMER_STRENGTH_ATTACK"
     << "\t" << hammer_strength_attack() << endl;
  os << "GUN_STRENGTH_ATTACK"
     << "\t" << gun_strength_attack() << endl;
  os << "BAZOOKA_STRENGTH_ATTACK"
     << "\t" << bazooka_strength_attack() << endl;
  os << "BUILDER_STRENGTH_DEMOLISH"
     << "\t" << builder_strength_demolish() << endl;
  os << "HAMMER_STRENGTH_DEMOLISH"
     << "\t" << hammer_strength_demolish() << endl;
  os << "GUN_STRENGTH_DEMOLISH"
     << "\t" << gun_strength_demolish() << endl;
  os << "BAZOOKA_STRENGTH_DEMOLISH"
     << "\t" << bazooka_strength_demolish() << endl;
  os << "NUM_ROUNDS_REGEN_BUILDER"
     << "\t" << num_rounds_regen_builder() << endl;
  os << "NUM_ROUNDS_REGEN_WARRIOR"
     << "\t" << num_rounds_regen_warrior() << endl;
  os << "NUM_ROUNDS_REGEN_FOOD"
     << "\t" << num_rounds_regen_food() << endl;
  os << "NUM_ROUNDS_REGEN_MONEY"
     << "\t" << num_rounds_regen_money() << endl;
  os << "NUM_ROUNDS_REGEN_WEAPON"
     << "\t" << num_rounds_regen_weapon() << endl;
  os << "BARRICADE_RESISTANCE_STEP"
     << "\t" << barricade_resistance_step() << endl;
  os << "BARRICADE_MAX_RESISTANCE"
     << "\t" << barricade_max_resistance() << endl;
  os << "MAX_NUM_BARRICADES"
     << "\t" << max_num_barricades() << endl;
}
  ...
  for (int i = 0; i < board_rows(); ++i)
  {
    ...
    for (int j = 0; j < board_cols(); ++j)
    {
      ...
      if (c.type == Building)
        ...
        os << 'M';
      else if (c.bonus == Food)
        os << 'F';
      else if (c.id != -1)
      {
        if (c.resistance == -1)
        {
          if (citizens[c.id].type == Builder)
            os << 'C'; // builder without barricade
          else
            os << 'W'; // warrior without barricade
        }
        else
        {
          if (citizens[c.id].type == Builder)
            os << 'c'; // buider with barricade
          else
            os << 'w'; // warrior with barricade
        }
      }
      else if (c.resistance != -1)
        os << 'b'; // barricade with no citizen
      else
        os << '.';
    }
    os << endl;
...
void Board::perform_attack(Citizen &ci, Citizen &ci2, set<int> &killed, vector<pair<pair<CitizenType, int>, int>> &citizens_to_regenerate)
{
  ...
  { // Dead!!!
    ...
    if (loser.type == Builder)
      scr[winner.player] += kill_builder_points();
    else
      scr[winner.player] += kill_warrior_points();
  }
}
                    ...
                    vector<pair<pair<CitizenType, int>, int>> &citizens_to_regenerate)
{
  ...
  if (killed.count(id))
    return false; // Citizen has been killed in this round
  ...
  if (c_type == Move)
  {
    ...
    else if (nc.weapon != NoWeapon)
    {
      ...
      { // Warrior: moves and takes strongest weapon. Weapon disappears
        weapon_to_regenerate.push_back({nc.weapon, num_rounds_regen_weapon()});
    ...
    else if (nc.resistance != -1)
    { // Barricade
      if (day)
      {
        if (nc.id != -1)
          return false; // Also citizen: not attack to rivals, do not step on friends
        else if (nc.b_owner != pl)
          return false; // No citizen and barricade of another rival at day
        else
        { // Same clan free barricade : moves there
          nc.id = id;
          oc.id = -1;
          ci.pos = np;
        }
      ...
      { // night
        if (nc.b_owner != pl)
        { // rival barricade --> demolish independently of whether there is a citizen
          nc.resistance -= weapon_strength_demolish(ci.weapon);
          if (nc.resistance <= 0)
          { // Barricade disappears
        ...
        { // own barricade
          if (nc.id != -1)
            return false; // occupied --> ignore move
          else
          { // free --> move there
            nc.id = id;
            oc.id = -1;
    ...
    { // Citizen (pero no barricada)
      if (day)
        return false;
      else if (citizens[nc.id].player == pl)
        return false; // Never attack same clan
      else
      { // Night and other clan: attack but not move!!!!
        perform_attack(ci, citizens[nc.id], killed, citizens_to_regenerate);
      }
    }
    else
    { // Free cell, move there
      nc.id = id;
      oc.id = -1;
      ci.pos = np;
    }
  ...
  else if (c_type == Build)
  {
    ...
    { // new barricade
      ...
      nc.resistance = min(nc.resistance + barricade_resistance_step(), barricade_max_resistance());
  }
...
pair<bool, Pos> Board::get_random_pos_where_regenerate()
{
  ...
  if (res.size() != 0)
    return {true, res[random(0, res.size() - 1)]};
  else
    return {false, Pos()};
}
...
void Board::regenerate_citizens(vector<pair<pair<CitizenType, int>, int>> &to_regen)
{
  ...
  for (auto p : to_regen)
  {
    ...
    { // Regenerate now
      ...
      new_to_regen.push_back(p); // To be regenerated later
  }
...
void Board::generate_buildings(int num_building_cells, int num_streets)
{
  ...
  while (num_streets_pending > 0)
  {
    ...
    if (num_streets_pending != 1)
      length = num_building_cells / num_streets_pending;
    else
      length = num_building_cells;
