...
{
  if (w1 == Bazooka or w2 == Bazooka)
    return Bazooka;
  else if (w1 == Gun or w2 == Gun)
    return Gun;
  else if (w1 == Hammer or w2 == Hammer)
    return Hammer;
  else
    return NoWeapon;
}
...
inline Cell::Cell() : type(Street), bonus(NoBonus), weapon(NoWeapon), resistance(-1), b_owner(-1), id(-1) {}
inline Cell::Cell(CellType t, BonusType b, WeaponType w, int r, int o, int i) : type(t), bonus(b), weapon(w), resistance(r), b_owner(o), id(i) {}

inline bool Cell::is_empty() const
{
  return type == Street and
         bonus == NoBonus and
         weapon == NoWeapon and
         resistance == -1 and
         b_owner == -1 and
